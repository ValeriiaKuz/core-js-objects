...
function makeWord(lettersObject) {
  ...
  Object.entries(lettersObject).forEach(([, positions]) => {
    totalLetters += positions.length;
  });
...
function sellTickets(queue) {
  ...
  queue.forEach((bill) => {
    ...
    if (bill === 100 && bill25 < 1) {
      result = false;
    }
    if (bill === 100 && bill25 < 3 && bill50 < 1) {
      result = false;
    }
    if (bill === 100 && bill50 > 0) {
      bill50 -= 1;
      bill25 -= 1;
    } else if (bill === 100) {
      bill25 -= 3;
...
function fromJSON(proto, json) {
  ...
  const specifiedObj = Object.create(proto);
  Object.assign(specifiedObj, obj);
  return specifiedObj;
}
